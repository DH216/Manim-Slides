...
class Scene2_4(Slide):
    def construct(self):
        ...
        img_x0 = ImageMobject("./Imgs/Scene4/ffhq_3.png").scale_to_fit_width(2)
        img_x0_rect = SurroundingRectangle(img_x0, color=WHITE, buff=0.0)
        img_xt_1 = ImageMobject("./Imgs/Scene4/ffhq_3_noise_61.png").scale_to_fit_width(2)
        img_xt_1_rect = SurroundingRectangle(img_xt_1, color=WHITE, buff=0.0)
        img_xt = ImageMobject("./Imgs/Scene4/ffhq_3_noise_102.png").scale_to_fit_width(2)
        img_xt_rect = SurroundingRectangle(img_xt, color=WHITE, buff=0.0)
        img_xT = ImageMobject("./Imgs/Scene4/pure_noise.png").scale_to_fit_width(2)
        img_xT_rect = SurroundingRectangle(img_xT, color=WHITE, buff=0.0)
        ...
        dots_x0_xt = VGroup(*[Dot(color=WHITE, z_index=4, radius=0.025) for _ in range(3)])
        dots_x0_xt.arrange(RIGHT, buff=0.05).move_to((x0g.get_right() + x1g.get_left()) / 2)
        
        dots_xt1_xT = VGroup(*[Dot(color=WHITE, z_index=4, radius=0.025) for _ in range(3)])
        dots_xt1_xT.arrange(RIGHT, buff=0.05).move_to((xtg.get_right() + xTg.get_left()) / 2)
        ...
        formula = MathTex(
            r"\mathbb{E}_q[f] = \int q(x) f(x) \, dx.",
            color=WHITE,
            font_size=32
        )
